...
# set factor levels
categorical_stats$value <- factor(categorical_stats$value, levels = unique(categorical_stats$value))
...
grdAmazonia$Biome_short <- as.factor(sapply(grdAmazonia$Biome, function(x) biome_names[x]))
...
boxplot(Terra_PP ~ Biome_short, data = grdAmazonia, 
        ...
        ylab = "Terra_PP",
        las = 1,  # x轴标签旋转
        col = "lightgreen",  # 箱线图颜色
        border = "darkgreen")  # 边框颜色
...
boxplot(Prec_dm ~ Biome_short, data = grdAmazonia, 
        ...
        ylab = "Prec_dm",
        las = 1,  # x轴标签旋转
        col = "lightgoldenrodyellow",  # 箱线图颜色
        border = "goldenrod")  # 边框颜色
...
ggcorrplot(cor_matrix, 
           ...
           type = "lower",  # Display the lower triangle of the correlation matrix
...
# Transform the data to sf target, specify the original projected coordinate system as World Sinusoidal.
grdAmazonia_sf <- st_as_sf(mysample, coords = c("x1", "x2"), crs = sinusoidal_crs)
...
# Figure 12
#####################################
# Figure 13:he Image of AGB with Samples Overlay Using Stratified Simple Random Sampling
data("grdAmazonia")
...
print(final_plot)
######################################
# Figure 14:Distribution of Biome with Samples Overlay Using Stratified Simple Random Sampling
# Convert Biome to a factor
grdAmazonia$Biome <- as.factor(grdAmazonia$Biome)

# Define a color palette for the biomes
biome_colors <- c("Mangroves" = "#ff7f00",  # Orange for Mangroves
                  "Forest_dry" = "purple",  # Purple for Tropical & Subtropical Dry Broadleaf Forests
                  "Grassland" = "#1f78b4",  # Blue for Tropical & Subtropical Grasslands, Savannas & Shrublands
                  "Forest_moist" = "#33a07c")  # Green for Tropical & Subtropical Moist Broadleaf Forests
...
print(final_plot)
#######################################
# Figure 15: The Image of AGB with Samples Overlay Using Convenience Sampling in One Circular Area
data(grdAmazonia)
...
#######################################
# Figure 16: The Image of AGB with Samples Overlay Using Convenience Sampling in Two Circular
Areas
# Load dataset
data(grdAmazonia)
...
ggplot(data, aes(x = SamplingMethod)) +
  ...
  geom_bar(aes(y = Coverage, fill = "Coverage"), stat = "identity", position = "dodge", width = 0.5) +
  # Line graph of Bias
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~ . * max(data$Bias), name = "Bias")  # Bias: secondary axes
  ) +
...
ggplot(data, aes(x = SamplingMethod, fill = SamplingCategory)) +
  # Bar graph for Coverage
  geom_bar(aes(y = Coverage), stat = "identity", position = "dodge", width = 0.5) +
  # Line graph of Bias
  geom_line(aes(y = Bias / max(data$Bias) * 1, group = 1), color = "red", size = 1) +
  ...
  labs(title = "Comparison of Model-Assisted Sampling Methods", x = "Sampling Method") +
  theme_minimal() +
  coord_flip() +  # Flip the axes
...
p1 <- ggplot(data, aes(x = Estimator, y = Bias, fill = Sampling_Method)) +
  ...
  labs(title = "Comparison of Bias for Different Sampling Methods with Model-Based Approach", 
       y = "Bias", x = "Estimator") +
  theme_minimal()
...
p2 <- ggplot(data, aes(x = Estimator, y = Coverage, fill = Sampling_Method)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Comparison of Coverage for Different Sampling Methods with Model-Based Approach", 
...
# data
data1 <- data.frame(
  Sampling_Method = rep(c("Design-based approach", "Model-assisted approach", "Model-based approach"), 
                        times = c(1, 2, 5)),
  Estimator = c("SRS estimator", 
                "Single regression estimator", "Multiple regression estimator", 
                "LM", "LM with interaction", "GLM", "GLM with interaction", "GAM"),
...
ggplot(data1, aes(x = Estimator)) +
  geom_bar(aes(y = Coverage, fill = Sampling_Method), stat = "identity", position = "dodge") +
  geom_text(aes(y = Coverage, label = round(Coverage, 2)), 
            position = position_dodge(width = 0.9), vjust = -0.5) +  # Add coverage's labels
  geom_line(aes(y = Bias / 50, group = Sampling_Method), color = "black", size = 1) +
  geom_point(aes(y = Bias / 50), color = "black", size = 2) +
  geom_text(aes(y = Bias / 50, label = round(Bias, 3)), 
            position = position_dodge(width = 0.9), vjust = -0.5) +  # Add bias's labels
  scale_y_continuous(
    name = "Coverage",
    sec.axis = sec_axis(~ . * 50, name = "Bias")
  ) +
  labs(title = "Comparison of Bias and Coverage for Different Estimation Approaches") +
  theme_minimal() +
...
ggplot(data2, aes(x = Estimator)) +
  geom_bar(aes(y = Coverage, fill = Approach), stat = "identity", position = "dodge") +
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~ . * (6 / 1), name = "Bias", breaks = seq(0, 6, by = 1))  # Bias (0-6)
  ) +
  labs(title = "Comparison of Bias and Coverage for Different Estimation Approaches") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
data3 <- data.frame(
  ...
  Estimator = c("SRS estimator", "Single regression estimator", "Multiple regression estimator", 
...
ggplot(data3, aes(x = Estimator)) +
  ...
  scale_y_continuous(
    name = "Coverage",
    limits = c(0, 1),  # Coverage:(0-1)
    sec.axis = sec_axis(~ . * (200 / 1), name = "Bias", breaks = seq(0, 200, by = 50))  # Bias:(0-200)
  ) +
  labs(title = "Comparison of Bias and Coverage for Different Estimation Approaches") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ggplot(data4, aes(x = Estimator)) +
  ...
  scale_y_continuous(
    name = "Coverage",
    limits = c(0, 1),  # Coverage :(0-1)
    sec.axis = sec_axis(~ . * (40 / 1), name = "Bias", breaks = seq(0, 40, by = 10))  # Bias:(0-40)
  ) +
  labs(title = "Comparison of Bias and Coverage for Different Estimation Approaches") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))